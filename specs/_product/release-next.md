...
## Constraints
- **YAML files ARE the ledger**. No `specs/narrative.jsonl`, no write-side `HandoverRecord` Pydantic model, no content-hash on write (per `specs/plans/deviate-content.md` lines 13-15). The YAML manifest under `.deviate/feat/` is the durable artifact and is re-emittable from skills if lost.
...
- **No commit on handover YAMLs**. `.deviate/feat/**` and `.deviate/content-drafts/**` are gitignored runtime state. Git log already captures code/test/spec changes; the YAML is synthesis material, not project source (per `specs/plans/deviate-content.md` lines 17-19).
...
- **Path traversal rejected at the `pathlib` boundary**. Any path that escapes `.deviate/feat/` is rejected by `handover_path()` and `persist_handover()` (per `specs/plans/deviate-content.md` § Risks, lines 162-163).
...
- `handover_path()` returns `.deviate/feat/<epic_slug>/<issue_id>/<phase>.yaml` for macro handovers and `.deviate/feat/<epic_slug>/<issue_id>/<task_id>/<phase>.yaml` for micro handovers (per `specs/plans/deviate-content.md` § Path convention, lines 44-45).
...
- `handover_path()` and `persist_handover()` reject any path that escapes `.deviate/feat/` via `pathlib` and raise a clear diagnostic (per `specs/plans/deviate-content.md` § Risks, lines 162-163).
...
- `deviate content --window EPIC-X` filters records to `.deviate/feat/<epic_slug>/**` only; absence of the window flag includes all records in chronological order (per `specs/plans/deviate-content.md` § Task 2, line 145).
...
- `.deviate/.gitignore` excludes `/feat/` and `/content-drafts/`; `git ls-files .deviate/feat/` returns no results for any phase that has completed since the gitignore change (per `specs/plans/deviate-content.md` lines 17-19 and § File changes → Modifications, lines 83-87).
- End-to-end smoke: a single DeviaTDD task (RED → GREEN → JUDGE → REFACTOR) produces `.deviate/feat/<epic>/<issue>/<task>/{red,green,judge,refactor}.yaml` with valid YAML, present on disk, and not staged in the git index (per `specs/plans/deviate-content.md` § Verification, line 189).
