...
* **Description:** Initializes a standard project-level DeviaTDD compliance framework. Builds
  ...
  **Agent-to-skills-directory mapping:** `--agent claude` → `.claude/skills/`; `--agent opencode` → `.opencode/skills/`; `--agent factory` and `--agent droid` both → `.factory/skills/` (the Factory Droid IDE owns that directory; `droid` is the underlying backend binary both user-facing names dispatch to, so there is no `.droid/skills/`); `--agent pi` → `~/.pi/agent/skills/` via symlinks generated by `deviate init` (Pi implements the [Agent Skills specification](https://agentskills.io/specification) natively and discovers skills from `~/.pi/agent/skills/`, `.pi/skills/`, and `.agents/skills/` on startup; the project skill vault `src/deviate/prompts/skills/<name>` is symlinked one-to-one into `~/.pi/agent/skills/<name>`, so no skill content is duplicated and the project vault remains the single source of truth — see DeviaTDD-architecture.md §10.2 for the full symlink + settings.json strategy). The corresponding `.gitignore` entry follows the same mapping.
