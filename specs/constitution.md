...
- **Tamper Guard & Micro-Sandboxing**: GREEN phase resets test directories to post-RED commit state before evaluation. Micro-layer LLM execution (Aider) is strictly sandboxed: it is granted write access **only** to files matching `src/**/*.py`. All `tests/`, `specs/`, and configuration files are strictly read-only during Micro-layer execution. Any mutation outside this allow-list triggers an immediate rollback. **Exception (Content Capture runtime state)** — micro-layer skill actors are permitted to write to `.deviate/content/handovers/**/*.yaml` and `.deviate/content/drafts/**/*.md` because both paths are gitignored runtime state owned by the skill actor's Write tool as part of FLOW-11 (Capture Phase Handover) and FLOW-12 (Synthesize Content Digest) per `specs/_product/architecture.md` §3.5-§3.7. These paths never participate in `git diff` evaluation (they are gitignored), so no source-tree rollback can fire. This exception is bounded to the two Content Capture path roots and to files written under them; any expansion requires a constitution amendment.
...
- 0.3.0 — Added Tamper Guard exception for Content Capture runtime state (`.deviate/content/handovers/**/*.yaml` and `.deviate/content/drafts/**/*.md`); resolved Constitutional Violation surfaced by Subagent Gamma in `/deviate-research` for epic 005-deviate-content (FLOW-11, FLOW-12); both paths are gitignored and do not participate in `git diff` evaluation
