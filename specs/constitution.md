...
- 0.3.0 — Added Tamper Guard exception for Content Capture runtime state (`.deviate/feat/**/*.yaml` and `.deviate/content-drafts/**/*.md`); resolved Constitutional Violation surfaced by Subagent Gamma in `/deviate-research` for epic 005-deviate-content (FLOW-11, FLOW-12); both paths are gitignored and do not participate in `git diff` evaluation
