...
## Corpus Check
- 167 files · ~153,518 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1460 nodes · 2253 edges · 59 communities (49 shown, 10 thin omitted)
- Extraction: 82% EXTRACTED · 18% INFERRED · 0% AMBIGUOUS · INFERRED: 396 edges (avg confidence: 0.72)
- Token cost: 148,000 input · 2,400 output
...
- `TestSessionState` --uses--> `TransitionViolationError`  [INFERRED]
  tests/test_state/test_config.py → src/deviate/state/config.py
- `TaskRecord` --uses--> `SessionState`  [INFERRED]
  tests/test_micro/test_run.py → src/deviate/state/config.py
- `TestMesoTaskLedger` --uses--> `SessionState`  [INFERRED]
  tests/test_integration/test_meso_task_ledger.py → src/deviate/state/config.py
- `TestExploreCommand` --uses--> `SessionState`  [INFERRED]
  tests/test_macro/test_explore.py → src/deviate/state/config.py
...
Cohesion: 0.06
Nodes (55): _extract_epic_num(), _halt(), _handle_transition_error(), _load_manifest(), Extract the leading numeric prefix from an epic slug.      ``001-deviate-cli-boo, Ensure .githooks/ is configured as hooks path if the directory exists., _run_pre_commit_hooks(), _validate_constitution() (+47 more)

### Community 3 - "Micro Layer TDD Sandbox"
Cohesion: 0.06
Nodes (39): _append_status_transition(), _dispatch_task(), _find_all_pending_tasks(), _find_task_record(), Run dispatcher: route task by execution_mode to TDD cycle or execute phase., Append a status-transition entry for a task.      Creates a new TaskRecord with, _read_ledger_records(), _resolve_issue_number() (+31 more)

### Community 4 - "Session & Config State"
Cohesion: 0.06
Nodes (25): Path, SessionState, _find_source_for(), reconstruct_from_worktree(), SessionState, validate_filesystem_state(), scaffold_artifacts(), TestMacroFullCycle (+17 more)

### Community 5 - "Meso Layer Specification"
Cohesion: 0.09
Nodes (46): _extract_issue_num(), _handle_missing_dot_dir(), Extract the numeric suffix from an issue ID.      ``ISS-006`` → ``006``, ``TSK-0, _derive_pr_metadata(), _emit_contract(), _find_spec_md(), _is_issue_completed(), _load_session() (+38 more)
...
Cohesion: 0.10
Nodes (19): emit_contract(), load_contract(), claim_issue(), is_issue_completed(), read_issue_body(), resolve_issue(), _resolve_ledger(), datetime (+11 more)
...
Cohesion: 0.42
Nodes (5): TaskRecord, _make_task_record(), TestRunCommand, _write_ledger(), Path
...
Cohesion: 0.44
Nodes (3): chdir(), TestFullInitCycle, Path
...
Cohesion: 0.67
Nodes (3): meso_workspace(), mock_workspace(), Path
...
## Knowledge Gaps
- **736 isolated node(s):** `embeddingProvider`, `/Users/werner/Development/tools/aether/pyproject.toml`, `/Users/werner/Development/tools/aether/mise.toml`, `/Users/werner/Development/tools/aether/CLAUDE.md`, `/Users/werner/Development/tools/aether/findings.md` (+731 more)
...
- **Why does `SessionState` connect `Session & Config State` to `PRD Command Tests`, `Core Utility Functions`, `Micro Layer TDD Sandbox`, `Research Command Tests`, `Meso Layer Specification`, `Init Scaffold & Governance`, `Shard Command Tests`, `Test Fixtures`, `Specify Command Tests`, `Task Run Command Tests`, `Explore Command Tests`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `chdir()` connect `Init Export Cycle Tests` to `PRD Command Tests`, `Research Command Tests`, `Micro Layer TDD Sandbox`, `Session & Config State`, `Test Fixtures`, `Shard Command Tests`, `Init Integration Tests`, `Specify Command Tests`, `Macro Contract Tests`, `Task Run Command Tests`, `Meso Contract Tests`, `Init Command Tests`, `Explore Command Tests`?**
...
- **What connects `embeddingProvider`, `/Users/werner/Development/tools/aether/pyproject.toml`, `/Users/werner/Development/tools/aether/mise.toml` to the rest of the system?**
